...
void ACMP::SetPlanarPriorParams() { params.planar_prior = true; }
...
void ACMP::CudaPlanarPriorInitialization(
...
int ACMP::GetReferenceImageWidth() { return cameras[0].width; }

int ACMP::GetReferenceImageHeight() { return cameras[0].height; }

cv::Mat ACMP::GetReferenceImage() { return images[0]; }

float4 ACMP::GetPlaneHypothesis(const int index) { return plane_hypotheses_host[index]; }

float ACMP::GetCost(const int index) { return costs_host[index]; }

float ACMP::GetMinDepth() { return params.depth_min; }

float ACMP::GetMaxDepth() { return params.depth_max; }
...
std::vector<Triangle> ACMP::DelaunayTriangulation(
