...
__version__ = None
exec(open("tmvs/version.py", "r").read())
...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        if platform.system() == "Windows":
            ...
            cmake_args += [
                f"-DCMAKE_LIBRARY_OUTPUT_DIRECTORY_{cfg.upper()}={str(Path(extdir) / 'tmvs')}"
            ]
            cmake_args += [
                f"-DCMAKE_RUNTIME_OUTPUT_DIRECTORY_{cfg.upper()}={str(Path(extdir) / 'tmvs')}"
            ]
            if sys.maxsize > 2**32:
                cmake_args += ["-A", "x64"]
            build_args += ["--", "/m"]
        else:
            cmake_args += [f"-DCMAKE_LIBRARY_OUTPUT_DIRECTORY={str(Path(extdir) / 'tmvs')}"]
            cmake_args += ["-DCMAKE_BUILD_TYPE=" + cfg]
            build_args += ["--", "-j8"]
        ...
        subprocess.check_call(
            ["cmake", ext.sourcedir, "-B", "build"] + cmake_args,
            cwd=self.build_temp,
            env=env,
        )
...
setup(
    ...
    package_dir={"tmvs": "tmvs"},
    ext_modules=[CMakeExtension(name="tmvs", library_dirs=[])],
